...
export async function POST(req: NextRequest) {
  ...
  // status: 'shipped' | 'ready_pickup' | 'cancelled'
  if (!orderId || !['shipped', 'ready_pickup', 'cancelled'].includes(status)) {
  ...
  // Actualizar estado. Si se cancela un pedido que ya estaba pagado, no lo
  // marcamos como "paid" a secas — refund_pending dice claramente que la
  // plata todavía no se devolvió (eso lo hace el dueño manualmente en MP o
  // por transferencia, no lo automatizamos acá).
  await service.from('orders').update({
    status,
    ...(trackingCode ? { tracking_code: trackingCode } : {}),
    ...(status === 'cancelled' && wasPaid ? { payment_status: 'refund_pending' } : {}),
  }).eq('id', orderId)
  ...
  const { data: config } = await service.from('store_config')
    .select('notification_email, email_from_name, email_intro_pedido_enviado, whatsapp_number')
  ...
  if (customerEmail && status === 'cancelled') {
    const subject = `Tu pedido fue cancelado — ${storeName}`
    const html = emailPedidoCancelado({
      storeName, orderId, customerName, wasPaid,
      whatsapp: cfg?.whatsapp_number ?? null,
      contactEmail: cfg?.notification_email ?? null,
    })
